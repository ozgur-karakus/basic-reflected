...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $note = $_POST['note'];
        try {
            $stmt = $pdo->prepare("INSERT INTO user_notes (user_id, note) VALUES (:user_id, :note)"); 
            $stmt->bindParam(':user_id', $userid);
            $stmt->bindParam(':note', $note);
            $stmt->execute();
            header("Location: main.php?user_id=$userid&message=success");
            exit;
    ...
    if (isset($_GET['delete'])) {
        ...
        try {
            ...
            $stmt->execute(); 
            header("Location: main.php?user_id=$userid&message=deleted");
            exit;
        } catch (PDOException $e) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM user_notes WHERE user_id = :user_id");
    ...
    <div class="container" style="margin-top: 20px;">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div>
                ...
                <a href="main.php" class="nav-link">Güvenli Not'a Git</a>
            </div>
        </div>
